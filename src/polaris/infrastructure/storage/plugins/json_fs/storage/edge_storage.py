...
from polaris.core.exceptions import EdgeNotFoundError, StorageError
...
class JsonEdgeStorage(EdgeStoragePlugin):
    ...
    async def backup(self, backup_dir: str) -> None:
        ...
        try:
            ...
            backup_file(self.edges_file)
        except Exception as e:
            logger.error(f"Failed to backup edges: {str(e)}")
            raise StorageError(f"Failed to backup edges: {str(e)}")
