...
from polaris.core.exceptions import NodeNotFoundError, StorageError
...
class JsonNodeStorage(NodeStoragePlugin):
    ...
    async def backup(self, backup_dir: str) -> None:
        ...
        try:
            ...
            backup_file(self.nodes_file)
        except Exception as e:
            logger.error(f"Failed to backup nodes: {str(e)}")
            raise StorageError(f"Failed to backup nodes: {str(e)}")
