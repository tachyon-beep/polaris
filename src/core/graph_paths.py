...
from typing import Callable, Dict, Iterator, List, Optional, Set, Tuple, TypeVar, Union, cast

from .exceptions import EdgeNotFoundError, GraphOperationError, NodeNotFoundError
...
class PathFinding:
    ...
    def _get_edge_weight(edge: Edge, weight_func: Optional[WeightFunc]) -> Weight:
        ...
        if weight <= 0:
            raise ValueError(
                f"Edge weight must be positive. Got {weight} for edge {edge.from_entity}->{edge.to_entity}"
            )
        return weight
