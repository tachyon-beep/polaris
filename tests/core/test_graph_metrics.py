...
from src.core.graph_metrics import GraphMetrics, MetricsCalculator, PathMetricsResult
...
def test_metrics_path_graph(path_graph):
    ...
    assert metrics.connected_components == 1
    assert metrics.diameter == None  # No complete paths between all nodes in directed graph
    # For path A->B->C->D:
...
def test_metrics_star_graph(sample_edge_metadata):
    ...
    assert metrics.clustering_coefficient == pytest.approx(0.0, abs=1e-10)  # No triangles
    assert metrics.connected_components == 1
    assert metrics.diameter == None  # No paths between leaf nodes in directed graph
